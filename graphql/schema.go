...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte utg address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an utg account at a particular block.
    type Account {
    ...
    # Transaction is an utg transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an utg account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an utg account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an utg block by number or by hash. If neither is
