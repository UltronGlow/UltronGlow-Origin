...
// Package graphql provides a GraphQL interface to utg node data.
package graphql
...
// Account represents an utg account at a particular block.
type Account struct {
...
// Transaction represents an utg transaction.
...
func (t *Transaction) GasPrice(ctx context.Context) (hexutil.Big, error) {
	...
	switch tx.Type() {
	case types.AccessListTxType, types.MultiSignerTxType:
...
type CallData struct {
	From     *common.Address // The utg address the call is from.
	To       *common.Address // The utg address the call is to.
