...
import (
	...
	"github.com/UltronGlow/UltronGlow-Origin/crypto/signify"
	"github.com/UltronGlow/UltronGlow-Origin/internal/build"
	"github.com/UltronGlow/UltronGlow-Origin/params"
)

var (
	// Files that end up in the utg*.zip archive.
	utgArchiveFiles = []string{
		"COPYING",
		executablePath("utg"),
	}

	// Files that end up in the utg-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert utg contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "utg bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (utg Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "utg",
			Description: "utg CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "utg private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "utg account management tool.",
		},
	}
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "utgstore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		env      = build.Env()
		baseutg = archiveBasename(*arch, params.ArchiveVersion(env.Commit))
		utg     = "utg-" + baseutg + ext
		alltools = "utg-alltools-" + baseutg + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(utg, utgArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{utg, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with utg-release.pub"
...
func doDockerImage(cmdline []string) {
	var (
		upload = flag.String("upload", "", `Where to upload the docker image (usually "utg/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
	...
	//  - utg/client-go:latest                            - Pushes to the master branch, utg only
	//  - utg/client-go:stable                            - Version tag publish on GitHub, utg only
	//  - utg/client-go:alltools-latest                   - Pushes to the master branch, utg & tools
	//  - utg/client-go:alltools-stable                   - Version tag publish on GitHub, utg & tools
	//  - utg/client-go:release-<major>.<minor>           - Version tag publish on GitHub, utg only
	//  - utg/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, utg & tools
	//  - utg/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, utg only
	//  - utg/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, utg & tools
	var tags []string
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "seaskycheng/utg")`)
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "utg-ci")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the utg sources.
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "utg-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "utg"
...
type debMetadata struct {
	...
	// utg version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "utg Builds <418012770@qq.com>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	// first section contains the utg binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"utg":     utgTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.utg.nsi", filepath.Join(*workdir, "utg.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("utg-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archive (usually "utgstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	tc := new(build.GoToolchain)
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.utg", "-v", "github.com/UltronGlow/UltronGlow-Origin/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("utg.aar", filepath.Join(GOBIN, "utg.aar"))
		os.Rename("utg-sources.jar", filepath.Join(GOBIN, "utg-sources.jar"))
		return
	}
	...
	archive := "utg-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("utg.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/UltronGlow/UltronGlow-Origin/mobile")
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "utg-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "utg.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "utg.podspec", "--allow-warnings")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "utgstore/builds")`)
