...
// Package consensus implements different utg consensus engines.
package consensus
...
type Engine interface {
	// Author retrieves the utg address of the account that minted the given
	...
	VerifyHeader(chain ChainHeaderReader, state *state.StateDB, header *types.Header, seal bool) error

	// VerifyHeaders is similar to VerifyHeader, but verifies a batch of headers
	// concurrently. The method returns a quit channel to abort the operations and
	// a results channel to retrieve the async verifications (the order is that of
	// the input slice).
	VerifyHeaders(chain ChainHeaderReader, state *state.StateDB, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error)
