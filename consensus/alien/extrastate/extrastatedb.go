...
import (
	...
	"github.com/UltronGlow/UltronGlow-Origin/core/state/snapshot"
...
// ExtraStateDB structs within the utg protocol are used to store anything
// within the merkle trie. ExtraStateDBs take care of caching and storing
...
type ExtraStateDB struct {
	...
	// grant account list
	grantList []common.Address // all grant accounts
...
func New(root common.Hash) (*ExtraStateDB, error) {
	tr, err := extradb.OpenTrie(root)
	if err != nil {
		log.Error("extrastate open trie", "err", err, "root", root)
		return nil, err
	}
	sdb := &ExtraStateDB{
		//state:        state,
...
func (s *ExtraStateDB) setError(err error) {
	if s.dbErr == nil {
		s.dbErr = err
	}
}
...
func (s *ExtraStateDB) Empty(addr common.Address) bool {
	...
	return so == nil || (so.empty() && so.lockempty())
}
...
func (s *ExtraStateDB) AddLockReward(addr common.Address, amount *big.Int) {
	stateObject := s.GetOrNewStateObject(addr)
	if stateObject != nil {
		//if stateObject.LockBalance().Cmp(common.Big0) == 0 {
			//s.grantListAppend(addr)
			s.lockAccounts.Append(addr)
		//}
...
func (s *ExtraStateDB) updateStateObject(obj *stateObject) {
...
func (s *ExtraStateDB) deleteStateObject(obj *stateObject) {
...
func (s *ExtraStateDB) getStateObject(addr common.Address) *stateObject {
...
func (s *ExtraStateDB) getDeletedStateObject(addr common.Address) *stateObject {
...
func (s *ExtraStateDB) setStateObject(object *stateObject) {
...
func (s *ExtraStateDB) createObject(addr common.Address) (newobj, prev *stateObject) {
...
func (s *ExtraStateDB) Copy() *ExtraStateDB {
	...
	state := &ExtraStateDB{
		...
		grantList:           make([]common.Address, 0),
		grantListhash:       common.Hash{},
		lockAccounts: newLockAccounts(),
		refund:              s.refund,
	...
	copy(state.grantList, s.grantList)
	state.grantListhash = s.grantListhash
...
func (s *ExtraStateDB) Finalise(deleteEmptyObjects bool) {
	...
	for addr := range s.journal.dirties {
		...
		if obj.suicided || (deleteEmptyObjects && obj.empty() && obj.lockempty()) {
			obj.deleted = true
		...
		if obj.lockempty() {
			//s.grantListDelete(obj.address)
			s.lockAccounts.Delete(obj.address)
		}
...
func (s *ExtraStateDB) IntermediateRoot(deleteEmptyObjects bool) common.Hash {
	...
	// modified after we start retrieving tries. Remove it from the ExtraStateDB after
	// this round of use.
	//
...
func (s *ExtraStateDB) clearJournalAndRefund() {
...
func (s *ExtraStateDB) Commit(deleteEmptyObjects bool) (common.Hash, error) {
	...
	if s.snap != nil {
		...
		// Only update if there's a state transition (skip empty Alien or Clique blocks)
...
func (s *ExtraStateDB) PayLockReward(number uint64, state *state.StateDB) []PayRecard {
	var payRecord []PayRecard
	for _, addr := range s.lockAccounts.Accounts() {
		obj := s.getStateObject(common.HexToAddress(addr))
		if obj == nil {
			log.Info("extrastate payreward addr not find", "addr", addr)
			continue
		}
		amount := obj.PayReward(number, state)
		if amount.Cmp(common.Big0) > 0 {
			payRecord = append(payRecord, PayRecard{
	...
	data, err := rlp.EncodeToBytes(payRecord)
	if err != nil {
		log.Error("extrastate rlp encode payRecord failed", "err", err)
		return payRecord
	}

	err = DBPut([]byte(fmt.Sprintf("payRecord-%d", number)), data)
	if err != nil {
		log.Error("extrastate putPayRecord", "error", err)
		return payRecord
	}
	return payRecord
}
...
func (s *ExtraStateDB) GetGrantListHash() common.Hash {
	return s.lockAccounts.Root()
	/*
	data, err := rlp.EncodeToBytes(s.grantList)
	if err != nil {
...
/*
func (s *ExtraStateDB) grantListAppend(addr common.Address) {
	s.grantList = append(s.grantList, addr)
}

func (s *ExtraStateDB) grantListDelete(addr common.Address) {
	for i, _ := range s.grantList {
		if s.grantList[i] == addr {
			s.grantList = append(s.grantList[:i], s.grantList[i+1:]...)
			break
		}
	}
}
...
func (s *ExtraStateDB) PutLockAccounts() (common.Hash, error) {
	return s.lockAccounts.Commit()
	/*
	data, err := rlp.EncodeToBytes(s.grantList)
	if err != nil {
		panic(fmt.Errorf("can't encode object at %v", err))
		//log.Error("extrastate putlockaccount", "err", err)
		return err
	}
	key := crypto.Keccak256Hash(data)
	err = DBPut(key.Bytes(), data)
	if err != nil {
		log.Error("extrastate dbput", "error", err)
		return err
	}
	log.Info("extrastate putlockaccounts", "accountsLen", len(s.grantList), "root", key)
	return nil
	 */
}

func (s *ExtraStateDB) LoadLockAccounts(root common.Hash) ([]common.Address, error) {
	return nil, s.lockAccounts.Load(root)
	/*
	var accounts []common.Address = []common.Address{}
	if root == (common.Hash{}) {
		return accounts, nil
	}
	data, err := DBGet(root.Bytes())
	if err != nil {
		log.Info("extrastate loadlockaccounts", "error", err, "root", root)
		return accounts, err
	}
	err = rlp.DecodeBytes(data, &accounts)
	if err != nil {
		log.Error("extrastate decode account", "error", err)
		return accounts, err
	}
	s.grantList = []common.Address{}
	s.grantList = append(s.grantList, accounts...)
	log.Info("extrastate load lockaccounts", "accountsLen", len(accounts), "root", root)
	return accounts, nil
	 */
}
/*
...
func LoadPayRecords(number uint64) []PayRecard {
	var payrecords []PayRecard = []PayRecard{}

	data, err := DBGet([]byte(fmt.Sprintf("payRecord-%d", number)))
	if err != nil {
		panic(fmt.Errorf("extrastate loadpayrecords, err = %v, number = %v",  err,  number))
	}

	err = rlp.DecodeBytes(data, &payrecords)
	if err != nil {
		panic(fmt.Errorf("extrastate decode payrecord, err = %v", err))
		return payrecords
	}
	log.Info("extrastate get payrecords", "number", number, "payrecords", payrecords)
	return payrecords
}
