...
import (
	...
	"github.com/UltronGlow/UltronGlow-Origin/accounts/keystore"
	"github.com/UltronGlow/UltronGlow-Origin/accounts/scwallet"
	...
	"github.com/UltronGlow/UltronGlow-Origin/consensus/alien"
	"github.com/UltronGlow/UltronGlow-Origin/consensus/clique"
	"github.com/UltronGlow/UltronGlow-Origin/consensus/ethash"
	"github.com/UltronGlow/UltronGlow-Origin/consensus/misc"
...
// PublicutgAPI provides an API to access utg related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicutgAPI struct {
	b Backend
}

// NewPublicutgAPI creates a new utg protocol API.
func NewPublicutgAPI(b Backend) *PublicutgAPI {
	return &PublicutgAPI{b}
}
...
func (s *PublicutgAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *PublicutgAPI) Syncing() (interface{}, error) {
...
// Sign calculates an utg ECDSA signature for:
// keccack256("\x19utg Signed Message:\n" + len(message) + message))
//
...
// the address of:
// hash = keccak256("\x19utg Signed Message:\n"${message length}${message})
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid utg signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and utg.
...
// PublicBlockChainAPI provides an API to access the utg blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new utg blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current utg chain config.
...
func (s *PublicBlockChainAPI) GetBlockByNumber2(ctx context.Context, number rpc.BlockNumber, fullTx bool) (map[string]interface{}, error) {
	block, err := s.b.BlockByNumber(ctx, number)
	if block != nil && err == nil {
		response, err := s.rpcMarshalBlock(ctx, block, true, fullTx)
		if err == nil && number == rpc.PendingBlockNumber {
...
type RPCTransaction struct {
	...
	Signatures       *types.SignerList `json:"SignatureList,omitempty"`
	Signers          []common.Address  `json:"signerList,omitempty"`
...
func newRPCTransaction(tx *types.Transaction, blockHash common.Hash, blockNumber uint64, index uint64, baseFee *big.Int) *RPCTransaction {
	...
	case types.MultiSignerTxType:
		al := tx.AccessList()
		result.Accesses = &al
		sl := tx.SignerList()
		result.Signatures = &sl
		result.Signers = tx.AllSigners()
		result.ChainID = (*hexutil.Big)(tx.ChainId())
...
// PublicDebugAPI is the collection of utg APIs exposed over the public
// debugging endpoint.
...
// TestSignAlienBlock fetches the given block number, and attempts to sign it as a alien header with the
// given address, returning the address of the recovered signature
//
// This is a temporary method to debug the externalsigner integration,
// TODO: Remove this method when the integration is mature
func (api *PublicDebugAPI) TestSignAlienBlock(ctx context.Context, address common.Address, number uint64) (common.Address, error) {
	block, _ := api.b.BlockByNumber(ctx, rpc.BlockNumber(number))
	if block == nil {
	...
	signature, err := wallet.SignData(account, accounts.MimetypeAlien, encoded)
	if err != nil {
		return common.Address{}, err
	}
	sealHash := alien.SealHash(header).Bytes()
	log.Info("test signing of alien block",
...
// PrivateDebugAPI is the collection of utg APIs exposed over the private
// debugging endpoint.
...
// Version returns the current utg protocol version.
