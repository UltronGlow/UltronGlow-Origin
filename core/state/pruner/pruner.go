...
func (p *Pruner) Prune(root common.Hash) error {
	...
	if blob := rawdb.ReadTrieNode(p.db, root); len(blob) == 0 {
		// The special case is for alien and clique based networks(rinkeby, goerli
...
pruning. Remember don't start the utg. without deleting the clean trie cache
otherwise the entire database may be damaged!

Check the command description "utg. snapshot prune-state --help" for more details.
`
