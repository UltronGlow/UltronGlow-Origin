...
// Package core implements the utg consensus protocol.
package core
...
// available in the database. It initialises the default utg Validator and
// Processor.
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	bc.engine.ApplyGenesis(bc.hc, bc.hc.CurrentHeader().Root)
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			//
			// But if the `TxIndexTail` is not nil, e.g. utg. is initialized with
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool) (int, error) {
	...
	for ; block != nil && err == nil || err == ErrKnownBlock; block, err = it.next() {
		...
		// Alien or Clique blocks where they can share state among each other, so importing an
		// older block might complete the state of the subsequent one. In this case,
		// just skip the block (we already validated it once fully (and crashed), since
		// its header and body was already in the database).
		if err == ErrKnownBlock {
			logger := log.Debug
			if bc.chainConfig.Alien == nil && bc.chainConfig.Clique == nil {
				logger = log.Warn
			}
			...
			// block in the middle. It can only happen in the alien or clique chain. Whenever
			...
			// Alien or Clique blocks to have the same state is if there are no transactions.
			lastCanon = block
			continue
		}
		...
		substart := time.Now()
		receipts, logs, usedGas, err := bc.processor.Process(block, statedb, bc.vmConfig,verifySeals)
		if err != nil {
...
// sync, utg. will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	defer bc.wg.Done()

	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init utg. with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded utg. to a new version which supports transaction
