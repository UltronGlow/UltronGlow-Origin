...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.utg", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for utg issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "utg-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/seaskycheng/utgtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for utg and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.utg", "github.com/UltronGlow/UltronGlow-Origin/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "utg.aar"), "utg.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "utg", "utgtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
