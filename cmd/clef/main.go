...
import (
	...
	"github.com/UltronGlow/UltronGlow-Origin/signer/core"
	"github.com/UltronGlow/UltronGlow-Origin/signer/fourbyte"
	"github.com/UltronGlow/UltronGlow-Origin/signer/rules"
	"github.com/UltronGlow/UltronGlow-Origin/signer/storage"
...
func init() {
	app.Name = "Clef"
	app.Usage = "Manage utg account operations"
	app.Flags = []cli.Flag{
		logLevelFlag,
		keystoreFlag,
		configdirFlag,
...
func testExternalUI(api *core.SignerAPI) {
	...
	{ // Sign data test - alien header
		api.UI.ShowInfo("Please approve the next request for signing a alien header")
		time.Sleep(delay)
		alienHeader := types.Header{
		...
		alienRlp, err := rlp.EncodeToBytes(alienHeader)
		if err != nil {
			utils.Fatalf("Should not error: %v", err)
		}
		addr, _ := common.NewMixedcaseAddressFromString("0x0011223344556677889900112233445566778899")
		_, err = api.SignData(ctx, accounts.MimetypeAlien, *addr, hexutil.Encode(alienRlp))
		expectApprove("signdata - alien header", err)
	}
