...
Use `devp2p enrdump <base64>` to verify and display an utg Node Record.
...
- `-utg-network <mainnet/rinkeby/goerli/ropsten>` filters nodes by "utg" ENR entry
- `-les-server` filters nodes by LES server support
- `-snap` filters nodes by snap protocol support

For example, given a node set in `nodes.json`, you could create a filtered set containing
up to 20 utg mainnet nodes which also support snap sync using this command:

    devp2p nodeset filter nodes.json -utg-network mainnet -snap -limit 20
...
### utg Protocol Test Suite

The utg Protocol test suite is a conformance test suite for the [utg protocol][utg].

To run the utg protocol test suite against your implementation, the node needs to be initialized as such:

1. initialize the utg node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run utg with the following flags:
```
utg --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the utg node: 
 ```
 devp2p rlpx utg-test <enode> cmd/devp2p/internal/ethtest/testdata/chain.rlp cmd/devp2p/internal/ethtest/testdata/genesis.json
```
...
To run the eth66 protocol test suite, initialize a utg node as described above and run the following command,
replacing `<enode>` with the enode of the utg node:

 ```
 devp2p rlpx eth66-test <enode> cmd/devp2p/internal/ethtest/testdata/chain.rlp cmd/devp2p/internal/ethtest/testdata/genesis.json
```

[utg]: https://github.com/seaskycheng/devp2p/blob/master/caps/utg.md
[dns-tutorial]: https://utg.org/docs/developers/dns-discovery-setup
[discv4]: https://github.com/seaskycheng/devp2p/tree/master/discv4.md
[discv5]: https://github.com/seaskycheng/devp2p/tree/master/discv5/discv5.md
