...
import (
	...
	"github.com/UltronGlow/UltronGlow-Origin/internal/utesting"
	"github.com/UltronGlow/UltronGlow-Origin/p2p"
	"github.com/UltronGlow/UltronGlow-Origin/p2p/rlpx"
)
...
func (s *Suite) dial66() (*Conn, error) {
	...
	conn.caps = append(conn.caps, p2p.Cap{Name: "utg", Version: 66})
...
func (c *Conn) handshake() error {
	...
	case *Hello:
		...
		if c.negotiatedProtoVersion == 0 {
			return fmt.Errorf("unexpected utg protocol version")
		}
		return nil
	default:
...
// negotiateutgProtocol sets the Conn's utg protocol version to highest
// advertised capability from peer.
func (c *Conn) negotiateEthProtocol(caps []p2p.Cap) {
	var highestEthVersion uint
	for _, capability := range caps {
		if capability.Name != "utg" {
			continue
		}
...
loop:
	...
	// make sure utg protocol version is set for negotiation
	if c.negotiatedProtoVersion == 0 {
		return nil, fmt.Errorf("utg protocol version must be set in Conn")
	}
	if status == nil {
