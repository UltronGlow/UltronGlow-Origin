...
// utg is the official command-line client for utg.
package main
...
import (
	...
	"github.com/UltronGlow/UltronGlow-Origin/console/prompt"
...
const (
	clientIdentifier = "utg" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the utg command line interface")
...
func init() {
	// Initialize the CLI app and start utg
	app.Action = utg
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2021 The utg Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.TestnetFlag.Name):
		log.Info("Starting utg on testnet...")

	case ctx.GlobalIsSet(utils.DeveloperFlag.Name):
		log.Info("Starting utg in ephemeral dev mode...")

	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting utg on utg mainnet...")
	}
...
// utg is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func utg(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend) {
	...
	// Create a client to interact with local utg node.
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full utg node is running
		if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		ethBackend, ok := backend.(*eth.EthAPIBackend)
		if !ok {
			utils.Fatalf("utg service not running: %v", err)
		}
