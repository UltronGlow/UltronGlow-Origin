...
// Package eth implements the utg protocol.
package eth
...
import (
	...
	"github.com/UltronGlow/UltronGlow-Origin/core/state/pruner"
	...
	"github.com/UltronGlow/UltronGlow-Origin/eth/protocols/eth"
	"github.com/UltronGlow/UltronGlow-Origin/eth/protocols/snap"
	...
	"github.com/UltronGlow/UltronGlow-Origin/p2p/dnsdisc"
...
// Ethereum implements the utg full node service.
...
// New creates a new utg object (including the
// initialisation of the common utg object)
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	// Assemble the utg object
	chainDb, err := stack.OpenDatabaseWithFreezer("chaindata", config.DatabaseCache, config.DatabaseHandles, config.DatabaseFreezer, "utg/db/chaindata/", false)
	if err != nil {
		return nil, err
	}
	...
	log.Info("Initialising utg protocol", "network", config.NetworkId, "dbversion", dbVer)

	if !config.SkipBcVersionCheck {
		if bcVersion != nil && *bcVersion > core.BlockChainVersion {
			return nil, fmt.Errorf("database version is v%d, utg %s only supports v%d", *bcVersion, params.VersionWithMeta, core.BlockChainVersion)
...
// APIs return the collection of RPC services the utg package offers.
