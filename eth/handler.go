...
// support all the operations needed by the utg chain protocols.
...
// newHandler returns a handler for all utg chain management protocol.
func newHandler(config *handlerConfig) (*handler, error) {
	...
	validator := func(header *types.Header) error {
		return h.chain.Engine().VerifyHeader(h.chain, nil,header, true)
	}
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("utg handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("utg peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("utg peer registration failed", "err", err)
		return err
	}
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("utg peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing utg peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("utg peer removal failed", "err", err)
	}
}
