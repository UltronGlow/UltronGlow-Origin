...
// Verify that Client implements the utg interfaces.
var (
	_ = utg.ChainReader(&Client{})
	_ = utg.TransactionReader(&Client{})
	_ = utg.ChainStateReader(&Client{})
	_ = utg.ChainSyncReader(&Client{})
	_ = utg.ContractCaller(&Client{})
	_ = utg.GasEstimator(&Client{})
	_ = utg.GasPricer(&Client{})
	_ = utg.LogFilterer(&Client{})
	_ = utg.PendingStateReader(&Client{})
	// _ = utg.PendingStateEventer(&Client{})
	_ = utg.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	if err != nil {
		t.Fatalf("can't create new utg service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == utg.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != utg.NotFound {
		t.Fatal("error should be utg.NotFound")
	}
}
