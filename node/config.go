...
import (
	...
	"github.com/UltronGlow/UltronGlow-Origin/accounts/external"
	"github.com/UltronGlow/UltronGlow-Origin/accounts/keystore"
	"github.com/UltronGlow/UltronGlow-Origin/accounts/scwallet"
	"github.com/UltronGlow/UltronGlow-Origin/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of utg is "utg". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "utg", keep that.
	if name == "utg" || name == "utg-testnet" {
		name = "utg"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "utg" instances.
var isOldutgResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by utg 1.4 are used if they exist.
	if warn, isOld := isOldutgResource[path]; isOld {
		oldpath := ""
		if c.name() == "utg" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldutgResourceWarning, "Using deprecated resource file %s, please move this file to the 'utg' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "utg-keystore")
		ephemeral = keydir
	}
